...
# --- FUNÇÃO DE FONTE PARA IMAGEM (LETRA GIGANTE) ---
def get_font_adaptavel(draw, texto, larg_max, alt_max):
    tamanho = 100  # Começa com tamanho enorme
    ...
    # Reduz apenas se o nome for realmente muito grande para a célula
    while draw.textlength(texto, font=font) > (larg_max - 30) and tamanho > 30:
        tamanho -= 2
        font = ImageFont.truetype(font_path, tamanho)
    return font
...
def gerar_imagem_escala(df):
    ...
    # Aumentamos a altura da linha para a fonte poder crescer verticalmente
    LARG_COL = 450  
    ALT_LINHA = 160 
    MARGEM_TOPO = 130
    ...
    f_titulo = ImageFont.truetype(font_path_bold, 80)
    ...
    for i, col in enumerate(colunas_foto):
        x = i * LARG_COL
        draw.rectangle([x, y, x + LARG_COL, y + 90], fill=(56, 142, 60), outline="white", width=2)
        txt_c = col.upper()
        f_c = get_font_adaptavel(draw, txt_c, LARG_COL, 90)
        w_c = draw.textlength(txt_c, font=f_c)
        draw.text((x + (LARG_COL - w_c)/2, y + 15), txt_c, fill="white", font=f_c)

    # Dados (Nomes das Irmãs com Fonte Grande)
    y += 90
    for idx_row, row in df.iterrows():
        ...
        for i, col in enumerate(colunas_foto):
            x = i * LARG_COL
            draw.rectangle([x, y, x + LARG_COL, y + ALT_LINHA], outline=(180, 180, 180), width=1)
            texto = str(row[col]).upper()
            if col == "Data": texto = texto.split('/')[0]
            
            f_n = get_font_adaptavel(draw, texto, LARG_COL, ALT_LINHA)
            w_n = draw.textlength(texto, font=f_n)
            draw.text((x + (LARG_COL - w_n)/2, y + 30), texto, fill="black", font=f_n)
        y += ALT_LINHA
...
# --- INTERFACE (TODOS OS CAMPOS RESTAURADOS) ---
def main():
        ...
        if aba == "📅 Gerar Rodízio" and area:
            ...
            if 'df_edit' in st.session_state:
                ...
                if st.button("💾 SALVAR E GERAR IMAGEM"):
                    ...
                    st.image(img)
                    st.download_button("📥 Baixar Imagem (Letra Grande)", img, "escala.png", "image/png")
            ...
            # Aqui você pode adicionar o formulário de cadastro de membros

        elif aba == "🚫 Afastamentos":
            st.header("🚫 Afastamentos e Restrições")
            restricoes = supabase.table("restricoes").select("*").execute()
            st.dataframe(pd.DataFrame(restricoes.data), use_container_width=True)

        elif aba == "⚙️ Configurar Áreas":
            st.header("⚙️ Áreas e Cargos")
