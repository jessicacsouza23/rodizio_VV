...
def main():
    ...
    if 'logged_in' not in st.session_state:
        ...
        if areas_res.data:
            sel = st.sidebar.selectbox("Escolha a Escala", [a['nome_area'] for a in areas_res.data])
            st.session_state['area_ativa'] = next(a for a in areas_res.data if a['nome_area'] == sel)
        
        aba = st.sidebar.radio("Navegação", ["Gerar Rodízio", "Histórico", "Gerenciar Membros", "Afastamentos", "Cargos"])
        if st.sidebar.button("Sair"): st.session_state.clear(); st.rerun()
        area = st.session_state.get('area_ativa')

        # --- ABA: GERAR RODÍZIO (COM REMOÇÃO DE DIAS ESPECÍFICOS) ---
        if aba == "Gerar Rodízio" and area:
            st.header(f"📅 Nova Escala: {area['nome_area']}")
            with st.container(border=True):
                c1, c2, c3 = st.columns(3)
                m = c1.selectbox("Quantos Meses?", [1,2,3,4,6])
                d_ini = c2.date_input("A partir de quando?")
                dias = c3.multiselect("Dias da Semana", DIAS_ORDEM, default=["Sunday"], format_func=lambda x: DIAS_TRADUCAO[x])
            ...
            if 'df_simulado' in st.session_state:
                ...
                st.subheader("🛠️ Ajuste Fino: Remova dias que NÃO terá culto")
                datas_disponiveis = df['Data'].tolist()
                remover = st.multiselect("Selecione as datas específicas para EXCLUIR (ex: feriados, reuniões):", datas_lista := datas_disponiveis)
                
                df_final = df[~df['Data'].isin(remover)].copy()
                st.dataframe(df_final.drop(columns=['_mes'], errors='ignore'), use_container_width=True)
                
                if st.button("💾 Salvar Escala Definitiva"):
                    ...
                    st.success("Salvo!"); del st.session_state['df_simulado']; st.rerun()
        ...
        # --- ABA: HISTÓRICO ---
            ...
            for e in h.data:
                with st.container(border=True):
                    ...
                    if c2.button("👁️ Ver", key=f"v_{e['id']}"): st.session_state['view_id'] = e['id']
        ...
        # --- ABA: GERENCIAR MEMBROS (DASHBOARD DE EDIÇÃO) ---
            ...
            with st.expander("➕ Adicionar Novo Membro"):
                with st.form("add_m"):
                    ...
                    if st.form_submit_button("Cadastrar"):
                        res = supabase.table("membros").insert({"nome": novo_n, "total_servicos": 0, "ultimo_servico": DATA_RESET_INTERNA}).execute()
            ...
            if ids:
                ...
                for m in ms.data:
                    with st.container(border=True):
                        ...
                        nome_edit = col1.text_input("Nome", value=m['nome'], key=f"edit_{m['id']}")
                        if col2.button("💾 Salvar Nome", key=f"btn_{m['id']}"):
                            supabase.table("membros").update({"nome": nome_edit}).eq("id", m['id']).execute()
                            st.success("Nome atualizado!"); st.rerun()
                        if col3.button("🗑️ Excluir", key=f"del_{m['id']}"):
            ...
            if ids:
                ...
                with st.form("afast"):
                    ...
                    if st.form_submit_button("Confirmar Afastamento"):
                        ...
                        st.success("Afastamento registrado!"); st.rerun()

        # --- ABA: CARGOS ---
        elif aba == "Cargos":
            st.header("⚙️ Configurar Escalas/Áreas")
            with st.form("area_f"):
                n = st.text_input("Nome (Ex: Organistas Manhã)"); v = st.number_input("Vagas", 1, 5, 2); p = st.text_input("Posições (Separadas por vírgula)")
